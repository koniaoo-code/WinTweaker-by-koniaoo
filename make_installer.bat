...
REM ---- 4) Deliver portable WinTweaker.exe and standalone WinTweaker_Setup.exe ----
echo  [*] Delivering WinTweaker.exe and WinTweaker_Setup.exe...
del "%HERE%\WinTweaker.exe" >nul 2>&1
del "%HERE%\WinTweaker_Setup.exe" >nul 2>&1
copy /y "%PUB%\WinTweaker.exe" "%HERE%\WinTweaker.exe" >nul
copy /y "%PUB%\WinTweaker.exe" "%HERE%\WinTweaker_Setup.exe" >nul
if not exist "%HERE%\WinTweaker.exe" goto :fail
if not exist "%HERE%\WinTweaker_Setup.exe" goto :fail
...
echo  ==========================================
echo    [OK] Done!
echo    - WinTweaker.exe (Portable)
echo    - WinTweaker_Setup.exe (Self-Installer)
echo  ==========================================
echo.
echo  Files are ready next to this script:
echo  * WinTweaker_Setup.exe - run to install shortcuts and uninstaller
echo  * WinTweaker.exe       - portable version (no installation)
echo.
...
if not defined ISCC if exist "%LOCALAPPDATA%\InnoSetup\ISCC.exe" set "ISCC=%LOCALAPPDATA%\InnoSetup\ISCC.exe"
