...
";
    ...
    /// <summary>Removes an app permanently (current user, all users, and ProvisionedPackage so updates won't bring it back).</summary>
    ...
    private static bool Remove(UwpApp app)
    {
        ...
        string script =
            ...
            $"Get-AppxProvisionedPackage -Online | Where-Object {{ $_.DisplayName -eq '{name}' -or $_.PackageName -like '*{name}*' }} | ForEach-Object {{ Remove-AppxProvisionedPackage -Online -PackageName $_.PackageName -ErrorAction SilentlyContinue }}";
